...
pessoas.nodes.forEach(e => {
    ...
    switch (e.type) {
        ...
        case 'estadual':
            ...
            e.cx = 368.5 + (Math.sin(degrees/180*Math.PI) * 308.611);
            e.cy = 383.491 - (Math.cos(degrees/180*Math.PI) * 308.611);
            arrayEstadual.push(e);
            break;
        case 'federal':
            var degrees = (e.degrees+13)/1.03;
            e.cx = 368.219 + (Math.sin(degrees/180*Math.PI) * 247.746);
            e.cy = 383.354 - (Math.cos(degrees/180*Math.PI) * 247.746);
            arrayFederal.push(e);
            var x = e.cx;
            var y = e.cy;
            break;
        case 'exterior':
            ...
            e.cx = 367.447 + (Math.sin(degrees/180*Math.PI) * 185.611);
            e.cy = 384.557 - (Math.cos(degrees/180*Math.PI) * 185.611);
            arrayExterior.push(e);
            break;
        case 'privada':
            var degrees = (e.degrees/1.09)+27;
            e.cx = 368.504 + (Math.sin(degrees/180*Math.PI) * 123.842);
